...
Get-ChildItem -Path $PSScriptRoot -File -Recurse -exclude *.ps1 | % {
    ...
    if ($contents -match "DocenterApi") {
        $contents -replace 'DocenterApi', $apiName | Set-Content $_.PSPath
        Write-Host $("'{0}': contents changed." -f $fileName)
    }

    if ($fileName -match 'DocenterApi') {
        $newName = $_.Name -replace 'DocenterApi', $apiName
...
Where-Object { $_.Name -match 'DocenterApi' } | % {
    Write-Host $("Editing directory: '{0}'." -f $_.FullName)
    $newDirName = $_.Name -replace 'DocenterApi', $apiName
